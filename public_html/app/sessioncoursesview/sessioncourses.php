...
function updateEntireCourse($mysqli, $type, $course) {
	try{
		...
		if ($type == 1) {
			...
			$data['staffs']  = updateCourseAttendance($mysqli, $course['staffsperm']);
			$data['staffs']  = updateCourseAttendance($mysqli, $course['staffsrepl']);
			$data['success'] = true;
		} else if ($type == 2) {
			$data['members'] = updateNumberAttendance($mysqli, $course['members']);
			$data['staffs']  = updateNumberAttendance($mysqli, $course['staffsperm']);
			$data['success'] = true;
		}
...
/**
 * This function gets the details of all permanent staff for a session course
 */
function getSessionCourseStaffs($mysqli, $type, $sessionscoursesid, $statuscode, $language) {
	if (empty($sessionscoursesid)) throw new Exception("Invalid session course.");
	if ($type == 1) {
		$query = "SELECT null registrationenddate, null registrationstartdate, cm.firstname, cm.lastname, cm.id memberid, getCodeDescription('staffcodes', cscs.staffcode, '$language') staffcodelabel
							FROM cpa_sessions_courses_staffs cscs
							JOIN cpa_members cm ON cm.id = cscs.memberid
							WHERE sessionscoursesid = $sessionscoursesid
							AND cscs.statuscode = '$statuscode'
							order by cscs.staffcode, cm.lastname";
	} else if ($type == 2 && $statuscode == 'PERM') {
		$query = "SELECT null registrationenddate, null registrationstartdate, cm.firstname, cm.lastname, cm.id memberid, 
										 getCodeDescription('staffcodes', csns.staffcode, '$language') staffcodelabel
...
function getCourseDetails($mysqli, $type, $id, $language) {
	try{
		if ($type == 1) {
			$query = "SELECT csc.*, getEnglishTextLabel(csc.label) as label_en, getFrenchTextLabel(csc.label) as label_fr, getTextLabel(cc.label, '$language') courselabel, getTextLabel(ccl.label, '$language') levellabel
								FROM cpa_sessions_courses csc
								JOIN cpa_courses cc ON cc.code = csc.coursecode
								LEFT JOIN cpa_courses_levels ccl ON ccl.coursecode = cc.code and ccl.code = csc.courselevel
								WHERE id = $id";
		...
		while ($row = $result->fetch_assoc()) {
			...
			$row['staffsperm'] 	= getSessionCourseStaffs($mysqli, $type, $row['id'], 'PERM', $language)['data'];
			if ($type == 1) {
				$row['staffsrepl'] 	= getSessionCourseStaffs($mysqli, $type, $row['id'], 'REPL', $language)['data'];
			}
			$data['data'][] = $row;
		}
		$data['success'] = true;
