...
function update_page($mysqli, $page) {
	...
	$query = "UPDATE cpa_ws_pages SET pageindex = $pageindex, navbarvisible = $navbarvisible, navbarusesection = $navbarusesection, navbarvisiblepreview = $navbarvisiblepreview, navbarusesectionpreview = $navbarusesectionpreview WHERE name = '$name'";
...
function updateEntireParagraphs($mysqli, $sectionname, $paragraphs) {
	...
	for ($x = 0; $paragraphs && $x < count($paragraphs); $x++) {
		...
		if ($mysqli->real_escape_string(isset($paragraphs[$x]['status'])) and $paragraphs[$x]['status'] == 'New') {
			$query = "INSERT INTO cpa_ws_sections_paragraphs (id, sectionname, paragraphindex, publish, visiblepreview, title, subtitle, paragraphtext)
								VALUES (null, '$sectionname', $paragraphindex, $publish, $visiblepreview, create_wsText('$title_en', '$title_fr'), create_wsText('$subtitle_en', '$subtitle_fr'),
