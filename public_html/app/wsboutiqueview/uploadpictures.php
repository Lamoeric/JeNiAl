...
*/
  ...
  $data['success'] = false;
  try {
    $mainobj = $_POST['mainobj']; // This is the good
    $id = (int)$mainobj['id'];
    // Set the directory names
    $uploads_dir    = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/goods/goodid' . $id . '/';
    $thumbnail_dir  = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/goods/goodid' . $id . '/thumbnails'.'/';
    ...
    if ($mysqli->query($query)) {
      ...
      if ($retVal != 0) {
        ...
        createThumbnail($uploads_dir, $partialfilename, $thumbnail_dir, 100);
      	...
      	$query = "UPDATE cpa_ws_goods_pictures
                  join (select max(cwgp.pictureindex)+1 pictureidx from cpa_ws_goods_pictures cwgp where cwgp.goodid = $id) a
