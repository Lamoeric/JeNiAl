...
function insert_contact($mysqli) {
	try {
		$data = array();
//		$id =					$mysqli->real_escape_string(isset($_POST['contact']['id	=']) 				? (int)$_POST['contact']['id'] : 0);
...
function update_contact($mysqli, $contact) {
	try {
		...
		$query = "UPDATE cpa_contacts SET lastname = '$lastname', firstname = '$firstname', homephone = '$homephone', 
																			cellphone = '$cellphone', officephone = '$officephone', officeext = '$officeext', email = '$email' 
							WHERE id = $id";
		if ($mysqli->query($query)) {
			$data['success'] = true;
...
function getAllContacts($mysqli, $filter) {
	try {
		...
		$query = "SELECT ca.id, ca.firstname, ca.lastname 
							FROM cpa_contacts ca " .$whereclause."
