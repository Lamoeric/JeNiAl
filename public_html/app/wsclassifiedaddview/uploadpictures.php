...
*/
  ...
  $data['success'] = false;
  try {
    $mainobj = $_POST['mainobj']; // This is the classified add
    $id = (int)$mainobj['id'];
    // Set the directory names
    $uploads_dir   = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/classifiedadds/addid' . $id . '/';
    $thumbnail_dir = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/classifiedadds/addid' . $id . '/thumbnails'.'/';

    // To create the filename, we need to insert the record in database
    $query = "INSERT INTO cpa_ws_classifiedadds_pictures(classifiedaddid, pictureindex, imagefilename)
              VALUES ($id, 0, '')";
    if ($mysqli->query($query)) {
      ...
      if ($retVal != 0) {
        ...
        createThumbnail($uploads_dir, $partialfilename, $thumbnail_dir, 100);
      	...
      	$query = "UPDATE cpa_ws_classifiedadds_pictures
                  join (select max(cwep.pictureindex)+1 pictureidx from cpa_ws_classifiedadds_pictures cwep where cwep.classifiedaddid = $id) a
