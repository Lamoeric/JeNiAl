...
//$ticketNotesFr = 'Ouverture des portes une demie-heure avant le spectacle. <br>Si�ge r�serv� - non remboursable';
...
if ($nbofsections == 0 || $nbofrows == 0 || $nbofseats == 0) {
	$pdf->AddPage('L');
	$html = '<b>Aucun billet � imprimer / No ticket to print</b>';
			...
			for ($z = 0; $z < $nbofseats; $z++) {
				...
				if (isSeatValid($sectionNo, $rowNo, $seatNo, $seating['exceptions'], $i) == true) {
					if ($firstTicket == true || $position == 6) {
						...
						$html = '<table><tr><td width="33%">JeNiAl</td><td width="33%" align="center">'.($i==1?'R�serv�':'').'</td><td align="right">'.$pagenumtxt.'</td></tr></table>';
