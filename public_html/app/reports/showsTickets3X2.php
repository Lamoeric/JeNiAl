...
for ($x = 0; $x < $nbofsections; $x++) {
	...
	for ($y = 0; $y < $nbofrows; $y++) {
		...
		for ($z = 0; $z < $nbofseats; $z++) {
			...
			if (isSeatValid($sectionNo, $rowNo, $seatNo, $seating['exceptions']) == true) {
				...
				if ($ticketWriteStdInfo == 1) {
					...
					$pdf->Image(K_PATH_PRIVATEIMAGES.PDF_HEADER_LOGO, $ticketLogo[$position]['x'], $ticketLogo[$position]['y'], $ticketLogo[$position]['w'], $ticketLogo[$position]['h'], 'JPG', '', '', false, 300, '', false, false, 1, false, false, false);
				...
				// Date and time
//				$pdf->writeHTMLCell($ticketPerfDate[$position]['w'], $ticketPerfDate[$position]['h'], $ticketPerfDate[$position]['x'], $ticketPerfDate[$position]['y'], $data['perfdate'].' '.$data['starttime'], 0, 1, 1, true, 'L', true);
...
*/
function isSeatValid($section, $row, $seat, $exceptions) {
	$return = true;
