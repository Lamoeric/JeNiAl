...
$data = getShowBillingList($mysqli, $showid, $language);
$billingList = $data['data'];

$data = getShowBillingTotals($mysqli, $showid, $language);
$billingSums = $data['data'][0];

// Page header
$pageheader = '<p align="center" style="font-size:20px"><b>'.$l['w_title'].' - '.$showlabel.'</h1></b>';
...
$billingSums['sumtotalamount'] = is_null($billingSums['sumtotalamount']) ? null : number_format($billingSums['sumtotalamount'], 2, ".", " ");
$billingSums['sumpaidamount'] = is_null($billingSums['sumpaidamount']) ? null : number_format($billingSums['sumpaidamount'], 2, ".", " ");
$billingSums['sumbalance'] = is_null($billingSums['sumbalance']) ? null : number_format($billingSums['sumbalance'], 2, ".", " ");
 ...
 */
function getShowBillingList($mysqli, $showid, $language) {
	try{
		if(empty($showid)) throw new Exception("Invalid session id.");
 ...
 */
function getShowBillingTotals($mysqli, $showid, $language) {
	try{
		if(empty($showid)) throw new Exception("Invalid session id.");
