...
// 	// echo $coaches;
	...
	if ($data['success'] == true && isset($data['data'])) {
		...
		testStarSessionTestSummarySheet($pdf, $tests, $testDirectorInfo, $language, $l);
	}
// }
...
function getStarTestSessionTests($mysqli, $language, $testsessionsid/*, $coaches*/){
  try{
    ...
    $data['success'] = null;
		...
		$query = "SELECT cnspr.*, ctd.type, candidate.firstname canfirstname, candidate.lastname canlastname,
											getTextLabel(cc2.label, '$language') candidatehomeclublabel,
											coach.firstname coafirstname, coach.lastname coalastname, coach.skatecanadano coaskatecanadano,
											...
											getTextLabel(cc.label, '$language') homeclublabel, cc.orgno, cnspr.result
							FROM cpa_newtests_sessions_periods_registrations cnspr
