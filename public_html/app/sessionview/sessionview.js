		...
		resolve: {
			auth: function ($q, authenticationService) {
				...
				if (userInfo) {
					...
					return $q.reject({authenticated: false, newLocation: "/sessionview"});
				}
			}
		}
	});
...
//					$scope.currentSession.sessionCourses[i].namenospace = $scope.currentSession.sessionCourses[i].name.replace(/[\s]/g, '_');
	...
	$scope.deleteFromDB = function(confirmed) {
		if ($scope.currentSession != null && !confirmed) {
			dialogService.confirmDlg($scope.translationObj.main.msgdeletesession, "YESNO", $scope.deleteFromDB, null, true, null);
		} else {
			$scope.promise = $http({
				method: 'post',
				url: './sessionview/sessions.php',
