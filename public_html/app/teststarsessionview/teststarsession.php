 ...
 * @throws Exception
 */
// function updateEntireRegistrations($mysqli, $registrations){
// 	$data = array();
// 	$data['inserted'] = 0;
// 	$data['updated'] 	= 0;
// 	$data['deleted'] 	= 0;
...
// 		// $approbationstatus = 					$mysqli->real_escape_string(isset( $registrations[$x]['approbationstatus']) 					? (int)$registrations[$x]['approbationstatus'] : 3);
...
// 		// 	$query = "INSERT INTO cpa_newtests_sessions_periods_registrations(id, newtestssessionsid, newtestssessionsperiodsid, coachid, memberid, testid, partnerid, musicid, createdby)
// 		// 						VALUES (null, $newtestssessionsid, $newtestssessionsperiodsid, $coachid, $memberid, $testid, $partnerid, $musicid, '$userid')";
// 		// 	if( $mysqli->query( $query ) ){
// 		// 		$data['inserted']++;
// 		// 	} else {
// 		// 		throw new Exception( $mysqli->sqlstate.' - '. $mysqli->error );
...
// 			$query = "update cpa_newtests_sessions_periods_registrations set result = $result where id = $id";
// 			if( $mysqli->query( $query ) ){
// 				$data['updated']++;
// 			} else {
// 				throw new Exception( $mysqli->sqlstate.' - '. $mysqli->error );
...
function update_testsession($mysqli, $testsession) {
	...
	$query = "UPDATE cpa_newtests_sessions
						...
						nbofdaysprior = $nbofdaysprior, testdirectorid = $testdirectorid, homeclub = '$homeclub' WHERE id = $id";
	if ($mysqli->query($query)) {
...
function getAllTestsessions($mysqli) {
	try {
		$query = "SELECT id, name FROM cpa_newtests_sessions ORDER BY id DESC";
...
function insertPeriods($mysqli, $testsessionPeriods) {
	try{
		...
		if (count($testsessionPeriods) > 0) {
			...
			// $data['updateflag'] = updateTestSessionGeneratedFlag($mysqli, $newtestssessionsid);
			$data['success'] = true;
		}
		$mysqli->close();
		echo json_encode($data);
	...
	try{
		...
		$query = "INSERT INTO cpa_newtests_sessions(id, name, label, testsessionstartdate, testsessionenddate, nbofdaysprior, testdirectorid, homeclub, periodsgenerated)
							 SELECT null, 'name/nom', create_systemtext(getEnglishTextLabel(label), getFrenchTextLabel(label)), testsessionstartdate, testsessionenddate, nbofdaysprior, testdirectorid, homeclub, 0
							 	FROM cpa_newtests_sessions WHERE id = $testsessionid";
		if ($mysqli->query($query)) {
			...
			if ($mysqli->query($query)) {
				$query = "INSERT INTO `cpa_newtests_sessions_charges`(`id`, `newtestssessionsid`, `chargecode`, `amount`)
										SELECT null, $id, `chargecode`, `amount`
											FROM cpa_newtests_sessions_charges WHERE newtestssessionsid = $testsessionid";
				if ($mysqli->query($query)) {
					$data['success'] = true;
					$query = "INSERT INTO `cpa_newtests_sessions_schedule`(`id`, `newtestssessionsid`, `arenaid`, `iceid`, `day`, `starttime`, `endtime`)
											SELECT null, $id, `arenaid`, `iceid`, `day`, `starttime`, `endtime`
											FROM `cpa_newtests_sessions_schedule` WHERE newtestssessionsid = $testsessionid";
					if ($mysqli->query($query)) {
						$data['success'] = true;
					} else {
	 			 		throw new Exception($mysqli->sqlstate.' - '. $mysqli->error);
