...
angular.module('core').service('arenaService', ['dialogService', '$http', function (dialogService, $http) {
	 ...
	 * @param {*} $scope 
	 * @param {*} language 
	 * @param {*} includenonactive if undefined or true, include non active arena in the list
	 */
	this.getAllArenas = function ($scope, language, includenonactive) {
		includenonactive = (includenonactive == undefined || includenonactive == true) ? 1 : 0;
		$http({
			method: 'post',
			url: './core/services/arena/getArenas.php',
			data: $.param({ 'language': language, 'includenonactive': includenonactive, 'type': 'getArenasDetails' }),
