...
angular.module('core').directive( "searchMemberButton", ['$uibModal', '$http', 'anycodesService', 'authenticationService', 'translationService', function($uibModal, $http, anycodesService, authenticationService, translationService) {
	return {
		require: '^form',				// To set the $dirty flag after copying the new member
		template:'<button class="btn btn-primary" ng-disabled="isFormPristine" id="searchmember"><span ng-if="isFullCopy==true">{{translationObj.searchmember.buttontitlesearch}}</span><span ng-if="isFullCopy==false">{{translationObj.searchmember.buttontitlecopy}}</span></button>',
		scope: {
			member: '=member',
