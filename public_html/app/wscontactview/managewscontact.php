...
function getContactDetails($mysqli, $fscname = '') {
	try {
		...
		$data = array();
		while ($row = $result->fetch_assoc()) {
			$filename = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/'.$row['logofilename'];
			// $filename = "../../website/images/".$row['logofilename'];
			$row['logofilenameexist'] = file_exists($filename);
			if (isset($row['logofilename']) && !empty($row['logofilename']) && file_exists($filename)) {
				$row['logoinfo'] = getimagesize($filename);
			}
			$filename = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/'.$row['sliderfilename'];
			// $filename = "../../website/images/".$row['sliderfilename'];
			if (isset($row['sliderfilename']) && !empty($row['sliderfilename']) && file_exists($filename)) {
				$row['sliderinfo'] = getimagesize($filename);
			}
			$data['data'][] = $row;
		}
		$mysqli->close();
