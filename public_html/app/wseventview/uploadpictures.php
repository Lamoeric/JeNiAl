...
*/
  ...
  try {
    ...
    $uploads_dir   = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/gallery/eventid' . $id . '/';
    $thumbnail_dir = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/gallery/eventid' . $id . '/thumbnails'.'/';
    ...
    if ($mysqli->query($query)) {
      ...
      if ($retVal != 0) {
        ...
        createThumbnail($uploads_dir, $partialfilename, $thumbnail_dir, 100);
      	...
      	$query = "UPDATE cpa_ws_events_pictures
                  join (select max(cwep.pictureindex)+1 pictureidx from cpa_ws_events_pictures cwep where cwep.eventid = $id) a
      ...
      // throw new Exception($mysqli->sqlstate.' - '. $mysqli->error . ' ' . $id);
      throw new Exception($mysqli->sqlstate.' - '. $mysqli->error);
    }
  } catch (Exception $e) {
    $data = array();
