...
if (isset($_POST['type']) && !empty(isset($_POST['type']))) {
	...
	switch ($type) {
		...
		case "delete_show":
			...
			break;
		case "getAllShows":
			getAllShows($mysqli, $_POST['language']);
			break;
		case "getShowDetails":
 ...
 * 
 * @param integer $showid	The id of the show from cpa_shows
 * @param integer $numberid	The id of the number from cpa_shows_numbers
 */
...
function updateEntireShowNumbers($mysqli, $showid, $numbers)
{
	...
	for ($x = 0; $x < count($numbers); $x++) {
		...
		if ($mysqli->real_escape_string(isset($numbers[$x]['status'])) and $numbers[$x]['status'] == 'New') {
			$query = "INSERT into cpa_shows_numbers (`id`, `showid`, `type`,`name`, `fees`, `label`, `duration`, `music`, `lights`, `comments`, `registrationtype`, 
													 `canbeinperformance`,`mandatory`, `practicesstartdate`, `practicesenddate`)
					  VALUES ('$id', '$showid', 1, '$name', '$fees', create_systemText('$label_en', '$label_fr'), $duration, '$music', '$lights', '$comments', $registrationtype, 
					  		  $canbeinperformance, $mandatory, "
		...
		if ($mysqli->real_escape_string(isset($numbers[$x]['status'])) and $numbers[$x]['status'] == 'Modified') {
			$query = "UPDATE cpa_shows_numbers 
					  SET name = '$name', fees = $fees, duration = $duration, music = '$music', lights = '$lights', comments = '$comments',
						registrationtype = $registrationtype, canbeinperformance = $canbeinperformance, mandatory = $mandatory,
						practicesstartdate = " . ($practicesstartdate == '' ? "null" : "'$practicesstartdate'")
...
function updateEntireShowPerformanceNumbers($mysqli, $showid, $performanceid, $numbers)
{
	...
	for ($x = 0; $x < count($numbers); $x++) {
		...
		$roomid = isset($numbers[$x]['roomid']) ? (int)$numbers[$x]['roomid'] : null;
...
function updateEntireShowPerformancePrices($mysqli, $showid, $performanceid, $prices)
{
	...
	for ($x = 0; $x < count($prices); $x++) {
		$id = 			isset($prices[$x]['id']) 		? (int)$prices[$x]['id'] : 0;
		$pricetype =	isset($prices[$x]['pricetype'])	? (int)$prices[$x]['pricetype'] : 0;
		...
		if ($mysqli->real_escape_string(isset($prices[$x]['status'])) and $prices[$x]['status'] == 'New') {
			...
			$query .= is_null($agemin) ? ", null" : ", '$agemin'";
			$query .= is_null($agemax) ? ", null" : ", '$agemax'";
			$query .= ")";
		...
		if ($mysqli->real_escape_string(isset($prices[$x]['status'])) and $prices[$x]['status'] == 'Modified') {
			...
			$query .= is_null($agemin) ? ", agemin = null" : ", agemin = '$agemin'";
			$query .= is_null($agemax) ? ", agemax = null" : ", agemax = '$agemax'";
			$query .= " WHERE id = '$id'";
...
function updateEntireShowPerformanceAssigns($mysqli, $showid, $performanceid, $assigns)
{
	...
	for ($x = 0; $x < count($assigns); $x++) {
		$id = 			$mysqli->real_escape_string(isset($assigns[$x]['id'])			? (int)$assigns[$x]['id'] : 0);
 ...
 */
function getAllShows($mysqli, $language)
{
	try {
		$query = "SELECT id, name, publish, active, getTextLabel(label, '$language') title, 
						 getCodeDescription('YESNO',publish, '$language') ispublish,
						 getCodeDescription('YESNO',active, '$language') isactive
				  FROM cpa_shows 
				  ORDER BY id desc";
...
function getShowPerformancePrices($mysqli, $performanceid, $language)
{
	...
	$data['data'] = array();
	while ($row = $result->fetch_assoc()) {
		$row['agemin'] = is_null($row['agemin']) ? null : (int)$row['agemin'];
		$row['agemax'] = is_null($row['agemax']) ? null : (int)$row['agemax'];
		$data['data'][] = $row;
	}
	$data['success'] = true;
	return $data;
...
function getShowPerformanceArenaExceptions($mysqli, $arenaid, $iceid, $language)
{
	$query = "SELECT group_concat(concat(caie.section, ' - ', caie.row, ' - ', caie.seat, ' - ', getCodeDescription('seatingexceptionreasons', caie.reason, '$language'))SEPARATOR ', ') exceptions
			  FROM cpa_arenas_ices_exceptions caie
			  WHERE caie.arenaid = $arenaid" . (isset($iceid) && !is_null($iceid) && $iceid != 0 ? " AND caie.iceid = $iceid" : "");
	$result = $mysqli->query($query);
	$data = array();
	$data['data'] = array();
	while ($row = $result->fetch_assoc()) {
	...
	if (count($data['data']) == 0) {
		$row['exceptions'] = $language == 'fr-ca' ? 'N/D' : 'N/A';
		$data['data'][] = $row;
	}
...
{
	if (empty($showid)) throw new Exception("Invalid show.");
	// $query_exceptions = "SELECT group_concat(concat(caie.section, ' - ', caie.row, ' - ', caie.seat) seating, getCodeDescription('seatingexceptionreasons', caie.reason, '$language') reasonlabel 
	// 					 FROM cpa_arenas_ices_exceptions caie
	...
	while ($row = $result->fetch_assoc()) {
		...
		$row['arenaexceptions'] =	getShowPerformanceArenaExceptions($mysqli, $row['arenaid'], $row['iceid'], $language)['data'][0]['exceptions'];
		$data['data'][] = $row;
	}
	$data['success'] = true;
	return $data;
		...
		if (count($numberPracticeDates) > 0) {
			...
			for ($x = 0; $x < count($numberPracticeDates); $x++) {
				...
				$query = "	SELECT * 
							...
							WHERE numberid = $numberid AND practicedate = '$practicedate' AND arenaid = $arenaid";
				$result = $mysqli->query($query);
