...
function delete_news($mysqli, $news) {
	try {
		...
		// Delete the filename related to the news
		$filename = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/news/'.$imagefilename;
...
function getAllNewss($mysqli, $language) {
	try {
		$query = "SELECT id, getWSTextLabel(title, '$language') title, publishdate FROM cpa_ws_news ORDER BY publishdate DESC";
...
function getNewsLinks($mysqli, $newsid, $language) {
	try {
		$query = "SELECT cwnl.*, getWSTextLabel(cwnl.label, 'fr-ca') label_fr, getWSTextLabel(cwnl.label, 'en-ca') label_en, getCodeDescription('wslinktypes', cwnl.linktype, '$language') linktypelabel,
...
function getNewsDetails($mysqli, $id, $language) {
	try {
		...
		while ($row = $result->fetch_assoc()) {
			...
			$data['data'][] = $row;
			$filename = '../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/news/'.$row['imagefilename'];
			if (isset($row['imagefilename']) && !empty($row['imagefilename']) && file_exists($filename)) {
				$data['imageinfo'] = getimagesize('../../../private/'. $_SERVER['HTTP_HOST'].'/website/images/news/'.$row['imagefilename']);
			}
		}
		$mysqli->close();
		$data['success'] = true;
...
function updateEntireLinks($mysqli, $newsid, $links) {
	...
	for ($x = 0; $links && $x < count($links); $x++) {
		...
		if ($mysqli->real_escape_string(isset($links[$x]['status'])) and $links[$x]['status'] == 'Modified') {
			$query = "UPDATE cpa_ws_news_links SET linkname = '$linkname', linktype = $linktype, linkpage = '$linkpage', linksection = '$linksection', linkdocumentid = $linkdocumentid,
