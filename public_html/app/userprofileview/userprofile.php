...
function update_user($mysqli, $user) {
	...
	if (empty($id)) {
		$data['insert'] = true;
		$query = "INSERT INTO cpa_users(id, code, description)
							VALUES (NULL, '$userid', '$username')";
		if ($mysqli->query($query)) {
			$data['success'] = true;
		...
		$query = "UPDATE cpa_users
							SET userid = '$userid', fullname = '$fullname', email = '$email', preferedlanguage = '$preferedlanguage', password = '$password'
							WHERE id = $id";
		if ($mysqli->query($query)) {
		} else {
			throw new Exception($mysqli->sqlstate.' - '. $mysqli->error);
...
function getUserDetails($mysqli, $id = '') {
	try{
		if (empty($id)) throw new Exception("Invalid User.");
		$query = "SELECT * FROM cpa_users WHERE id = $id";
		$result = $mysqli->query($query);
		$data = array();
		while ($row = $result->fetch_assoc()) {
